...
# ─────────────────────────────────────────
# 初期設定
# (page_configはルーター側のapp.pyで一元設定するため、ここでは行わない)
# ─────────────────────────────────────────
...
def load_section_standings() -> pd.DataFrame:
    ...
    while True:
        res = (
            client.table("team_section_standings")
            .select("*")
            .range(start, start + page_size - 1)
            .execute()
        )
...
st.caption(f"{season}シーズン {competition} ／ Team Basis")

# ─────────────────────────────────────────
# メイン: 試合情報 (対戦済み結果 or 対戦予定)
# ─────────────────────────────────────────

st.subheader("試合情報")
if match_is_finished:
    score_text = f" {match_row['score']} " if pd.notna(match_row.get("score")) else " vs "
    st.success(f"**{match_section}** で対戦済み：{home_team}{score_text}{away_team}")
else:
    st.info(f"**{match_section}** で対戦予定：{home_team}（ホーム） vs {away_team}（アウェイ）")
